function [polyCoeff_bor, polyCoeff_prebor] = ...
    PadeCoef_poly(nu, mu, deg_bor, deg_prebor, constant_condition)
...
%     constant_condition = 1;
    if (constant_condition == 1) && (mod(totalDeg_1, 2) == 0)
        error('Number of coefs in Border Deg must be odd');
    elseif (constant_condition == 1) && (mod(totalDeg_2, 2) == 0)
        error('Number of coefs in PreBorder Deg must be odd');
    elseif (constant_condition == 0) && (mod(totalDeg_1, 2) == 1)
        error('Number of coefs in Border Deg must be even');
    elseif (constant_condition == 0) && (mod(totalDeg_2, 2) == 1)
    ...
    for i = 0 : step_num_2 - 1
        ...
        for k = 0 : deg_prebor(i+1)
            ...
            switch constant_condition
                case 0
                    one_bound = num_unknCoeff_prebor/2 - 1;
                    two_bound = num_unknCoeff_prebor/2 - 1;
                    A_prebor(3:3+one_bound-1, sum(deg_prebor(1:i)+1) + k+1) = ...
                        (tmp_conv_3(end:-1:end-one_bound+1));
                    A_prebor(3+one_bound:end, sum(deg_prebor(1:i)+1) + k+1) = ...
                        (tmp_conv_4(end:-1:end-two_bound+1));
                case 1
                    one_bound = (num_unknCoeff_prebor - 1)/2 - 1;
                    two_bound = (num_unknCoeff_prebor - 1)/2 - 1;
                    A_prebor(4:4+one_bound-1, sum(deg_prebor(1:i)+1) + k+1) = ...
                        (tmp_conv_3(end:-1:end-one_bound+1));
                    A_prebor(4+one_bound:end, sum(deg_prebor(1:i)+1) + k+1) = ...
                        (tmp_conv_4(end:-1:end-two_bound+1));
            end
        end
    end
