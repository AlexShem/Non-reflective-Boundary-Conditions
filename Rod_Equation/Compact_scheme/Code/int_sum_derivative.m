function [I, K_int, P_int] = int_sum_derivative(U, dx, dt, rho, R, E)
...
%     dtU(:, 1) = []; dtU(:, end) = [];
    ...
    K = rho * dtU(:, 2:end-1).^2 + rho*R^2 * dxdtU.^2;
    K_0 = rho * dtU(:, 1).^2 + rho*R^2 * dxdtU_0.^2;
    K_N = rho * dtU(:, end).^2 + rho*R^2 * dxdtU_N.^2;
    
    P = E*R^2 * d2xU.^2;
    P_0 = E*R^2 * d2xU(:, 1).^2;
    P_N = E*R^2 * d2xU(:, end).^2;
    
    K_int = (dx * (.5*(K_0 + K_N) + sum(K, 2)));
    P_int = (dx * (.5*(P_0 + P_N) + sum(P, 2)));
    
    I = (dx * (.5*(theta_0 + theta_N) + sum(theta, 2)));
end