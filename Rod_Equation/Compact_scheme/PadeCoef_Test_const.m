function [polyCoeff_bor, polyCoeff_prebor] = PadeCoef_Test_const(nu, mu, deg_bor, deg_prebor)
    ...
    [~, ~, lam_ser_3, lam_ser_4] = lambda_series_sym(w, nu, mu,...
        max([totalDeg_1, totalDeg_2]), max([totalDeg_1, totalDeg_2]));
    ...
    if nu == 20100 && mu == 10000
        ...
        elseif isequal(deg_bor, [1,1,2,2])
            ...
            cf_bor_3 = coeffs(expand(eq_bor_3), w); display('1. border lam 3 ready');
            cf_bor_4 = coeffs(expand(eq_bor_4 * w^(step_num_1 - 1)), w); display('2. border lam 4 ready');
        end
    elseif nu == 9000 && mu == 10000
        if isequal(deg_bor, [0,0,2,2])
            load('border_cfs_9k_10k_0022.mat');
        elseif isequal(deg_bor, [1,1,2,2])
            load('border_cfs_9k_10k_1122.mat')
        else
            cf_bor_3 = coeffs(expand(eq_bor_3), w); display('1. border lam 3 ready');
            cf_bor_4 = coeffs(expand(eq_bor_4 * w^(step_num_1 - 1)), w); display('2. preborder lam 4 ready');
        end
    else
        cf_bor_3 = coeffs(expand(eq_bor_3), w); display('1. border lam 3 ready');
        cf_bor_4 = coeffs(expand(eq_bor_4 * w^(step_num_1 - 1)), w); display('2. preborder lam 4 ready');
    ...
    if nu == 20100 && mu == 10000
        ...
        elseif isequal(deg_prebor, [1,1,2,2])
            load('preborder_cfs_1122.mat');
        else
            cf_prebor_3 = coeffs(expand(eq_prebor_3), w); display('3. preborder lam 3 ready');
            cf_prebor_4 = coeffs(expand(eq_prebor_4 * w^(step_num_2 - 1)), w); display('4. preborder lam 4 ready');
        end
    elseif nu == 9000 && mu == 10000
        if isequal(deg_prebor, [0,0,2,2])
            load('preborder_cfs_9k_10k_0022.mat');
        elseif isequal(deg_prebor, [1,1,2,2])
            load('preborder_cfs_9k_10k_1122.mat');
        else
            cf_prebor_3 = coeffs(expand(eq_prebor_3), w); display('3. preborder lam 3 ready');
            cf_prebor_4 = coeffs(expand(eq_prebor_4 * w^(step_num_2 - 1)), w); display('4. preborder lam 4 ready');
        end
