...
%     N_eta = 20;
    ...
    eta_1 = sym2poly(eta_1); eta_1 = eta_1(4:end); eta_1 = poly2sym(eta_1, w);
...
%     N_lam_13_leg = 20;
    ...
    lam_ser_1 = eta_1/2 - sqrt(1/(4*a^2)-1) * (a^2/(1-4*a^2)*r.^2 - 2*a/(1-4*a^2)*r + 1) .* lam_13_leg;
    lam_ser_3 = eta_1/2 + sqrt(1/(4*a^2)-1) * (a^2/(1-4*a^2)*r.^2 - 2*a/(1-4*a^2)*r + 1) .* lam_13_leg;
...
%     N_lam_13_leg = 10;
    ...
    mul = 1;
    for n = 0 : N_lam
        lam_24_leg = lam_24_leg + mul * mul_fac / (1 - 2*n) * (w * eta_1 * fraction) ^ (2*n);
        mul = mul * (A*d)^2;
        mul_fac = mul_fac * (2*n + 1) * (2*n + 2) / (n + 1)^2;
    end
    lam_ser_2 = eta_2 / 2 * (1 - lam_24_leg);
    lam_ser_4 = eta_2 / 2 * (1 + lam_24_leg);
end