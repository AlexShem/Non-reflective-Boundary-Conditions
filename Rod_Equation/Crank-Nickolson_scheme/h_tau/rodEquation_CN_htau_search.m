...
for n = 3 : Nt
    ...
    rightPart(1) = rightPart(1) + sum(cf_rem_bor(:) .* U_prev_left_bor(:));
    rightPart(2) = rightPart(2) + sum(cf_rem_prebor(:) .* U_prev_left_prebor(:));
    rightPart(end-1) = rightPart(end-1) + sum(cf_rem_prebor(:) .* U_prev_right_prebor(:));
    rightPart(end) = rightPart(end) + sum(cf_rem_bor(:) .* U_prev_right_bor(:));
