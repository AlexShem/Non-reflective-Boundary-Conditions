function U = rodEquation_CN_Dirichlet()
...
%     U_at_ppTime = diag(ones(1, Nx-4)) + diag(a * ones(1, Nx-5), 1) + diag(a * ones(1, Nx-5), -1);
    ...
    % For Neuman
    U_now = diag(a*ones(1, Nx-4)) + diag(b * ones(1, Nx-5), 1) + diag(b * ones(1, Nx-5), -1)+ diag(s * ones(1, Nx-6), 2) + diag(s * ones(1, Nx-6), -2);
    
    U_at_pTime = diag(d * ones(1, Nx-4)) + diag(g * ones(1, Nx-5), 1) + diag(g * ones(1, Nx-5), -1);
